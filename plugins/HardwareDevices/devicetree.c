...
const DEVICE_PROPERTY_TABLE_ENTRY DeviceItemPropertyTable[] =
{
    ...
    { PhDevicePropertyManufacturer, L"制造商", TRUE, 180, DT_CENTER },
    { PhDevicePropertyService, L"服务", TRUE, 120, DT_CENTER },
    { PhDevicePropertyClass, L"类型", TRUE, 120, DT_CENTER },
    { PhDevicePropertyEnumeratorName, L"枚举器", TRUE, 80, DT_CENTER },
    { PhDevicePropertyInstallDate, L"安装时间", TRUE, 160, 0 },

    { PhDevicePropertyFirstInstallDate, L"首次安装", FALSE, 160, 0 },
    { PhDevicePropertyLastArrivalDate, L"上次就绪", FALSE, 160, 0 },
    ...
    { PhDevicePropertyPDOName, L"设备路径", FALSE, 180, DT_PATH_ELLIPSIS },
    ...
    { PhDevicePropertyFirmwareRevision, L"固件修订版本", FALSE, 80, 0 },
    { PhDevicePropertyHasProblem, L"故障", FALSE, 80, 0 },
    ...
    { PhDevicePropertyBusNumber, L"总线号", FALSE, 80, 0 },
    ...
    { PhDevicePropertyExtendedAddress, L"拓展地址", FALSE, 80, 0 },
    ...
    { PhDevicePropertyFirmwareVendor, L"固件容器", FALSE, 80, 0 },
    { PhDevicePropertySessionId, L"节点 ID", FALSE, 80, 0 },
    ...
    { PhDevicePropertyDriverProvider, L"驱动提供商", FALSE, 80, 0 },
    ...
    case MainTabPageInitializeSectionMenuItems:
        {
            ...
            menu = PhCreateEMenuItem(0, 0, L"硬件设备", NULL, NULL);
            PhInsertEMenuItem(menuInfo->Menu, menu, menuInfo->StartIndex);

            PhInsertEMenuItem(menu, PhPluginCreateEMenuItem(PluginInstance, 0, 98, L"折叠全部", NULL), ULONG_MAX);
            PhInsertEMenuItem(menu, PhPluginCreateEMenuItem(PluginInstance, 0, 99, L"展开全部", NULL), ULONG_MAX);
            PhInsertEMenuItem(menu, PhCreateEMenuSeparator(), ULONG_MAX);
            ...
            PhInsertEMenuItem(menu, PhCreateEMenuSeparator(), ULONG_MAX);
            PhInsertEMenuItem(menu, showDisconnectedDevices = PhPluginCreateEMenuItem(PluginInstance, 0, 102, L"显示已断开的设备", NULL), ULONG_MAX);
            ...
            PhInsertEMenuItem(menu, showDisabledDeviceInterfaces = PhPluginCreateEMenuItem(PluginInstance, 0, 105, L"显示已禁用的设备", NULL), ULONG_MAX);
            PhInsertEMenuItem(menu, sortChildDevices = PhPluginCreateEMenuItem(PluginInstance, 0, 106, L"排列子设备", NULL), ULONG_MAX);
            PhInsertEMenuItem(menu, sortRootDevices = PhPluginCreateEMenuItem(PluginInstance, 0, 107, L"排列父设备", NULL), ULONG_MAX);
            PhInsertEMenuItem(menu, highlightUpperFiltered = PhPluginCreateEMenuItem(PluginInstance, 0, 108, L"高亮较高过滤项", NULL), ULONG_MAX);
            PhInsertEMenuItem(menu, highlightLowerFiltered = PhPluginCreateEMenuItem(PluginInstance, 0, 109, L"高亮较低过滤项", NULL), ULONG_MAX);
