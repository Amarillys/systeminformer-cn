    ...
    )
{
    ...
    if (numberOfNetworkItems != 0)
    {
        ...
        menu = PhCreateEMenu();
        PhInsertEMenuItem(menu, PhCreateEMenuItem(0, ID_NETWORK_GOTOPROCESS, L"跳转到进程(&G)\bEnter", NULL, NULL), ULONG_MAX);
        PhInsertEMenuItem(menu, PhCreateEMenuItem(0, ID_NETWORK_GOTOSERVICE, L"跳转到服务", NULL, NULL), ULONG_MAX);
